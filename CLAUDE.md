...
43. **移动端适配** — CSS 三断点响应式布局（900px：工具栏折叠 + 侧边栏内嵌流式、640px：全宽面板、480px：44px 触控目标），工具栏水平滚动按钮（`overflow-x: auto` + 隐藏滚动条），`.stories-panel` 等面板 `max-width: 100vw` + `calc(100vw - 48px)`（`style.css`）
44. **文档统计** — `showStats` ref + `docStats` computed 计算 6 项统计（字数/字符数/段落/页数/图片/表格），折叠面板 `v-if="showStats"` 切换，基于 ParsedDocument.paragraphs 实时计算（`DocPreview.vue`）
45. **国际化 i18n** — 轻量级 i18n 方案，`locale.ts` 含 `chn`/`en` 双字典（180+ 条目），`shallowRef<Locale>` + `t()` 响应式函数，`tMap()` 支持嵌套键批量访问；App.vue 语言切换按钮 + localStorage 持久化；DocPreview.vue 全部模板字符串替换为 `t()` 调用（`locale.ts` + `App.vue` + `DocPreview.vue`）
46. **加载进度条** — `progressTrack`/`progressBar` CSS 动画（`@keyframes progressIndeterminate` + `progressShimmer` 双重动画），`progressStep` ref 阶段文字提示（读取/下载/解析/渲染），异步阶段 `progressStep.value = t('loading.progress.xxx')` 切换（`DocPreview.vue` + `locale.ts`）
47. **解析百分比进度** — `parseWithFormat(onProgress?)` 支持 `ProgressCallback` 回调穿透到 `extractFormattedText`，10 阶段进度报告（verifying/parsing_fib/parsing_clx/parsing_formats/parsing_fields/parsing_shapes/building_paragraphs/extracting_properties/extracting_images/finalizing）；`docParser.worker.ts` 消息协议升级为 `{type:'progress',stage,percent}` | `{type:'result',...}` 双类型；`parseWithWorker.ts` 按 `type` 字段分流，进度消息调用 `onProgress` 回调，结果消息 resolve Promise（`docParser.ts` + `docParser.worker.ts` + `parseWithWorker.ts`）
48. **错误分类与重试** — `errorClassifier.ts` 6 类错误分类（network/format/parse/corrupted/memory/unknown），每类含 `title`/`detail`/`suggestions`/`retryable` 字段，关键字匹配（'failed to fetch'/'cors'/'http 4xx'/'.docx'/'ole signature'/'worddocument'/'out of memory'/'parse' 等）；`ErrorDisplay.vue` 组件展示分类图标 + 标题 + 详细描述 + 解决建议列表 + 重试按钮（emit retry）+ 技术细节折叠（`<details>`）；DocPreview.vue `lastSource` ref 记录上次加载源，`retryLoad()` 函数重试（`errorClassifier.ts` + `ErrorDisplay.vue` + `DocPreview.vue`）
49. **组件拆分** — DocPreview.vue 拆分出 5 个子组件：`CollapsiblePanel.vue`（通用折叠面板，`modelValue`/`title`/`summary`/`panelClass` props + slot）、`DocStatsPanel.vue`（6 项统计）、`ShortcutsPanel.vue`（13 条快捷键 + 模态遮罩）、`LoadingOverlay.vue`（支持 indeterminate 和百分比两种模式，`progressPercent` prop）、`ErrorDisplay.vue`；减少 DocPreview.vue 代码量，为后续面板迁移到 CollapsiblePanel 奠定基础
...
- macOS `textutil` 创建的 .doc 文件 byte 10 为 0xBF，fComplex 不可靠 — 已通过 `isTextutilFib()` 检测并自动优化编码选择
